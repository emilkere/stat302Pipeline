...
use source() to source code directly from from the Code subfolde
```{r}
# load the code from source file
...
ggbox <- plot_rf_cv_df %>% ggplot(aes(x = as.factor(k), y = mse, group =
  as.factor(k))) + geom_boxplot(fill = "light blue") +
  scale_x_discrete(
